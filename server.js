...
// Uploaded custom backgrounds (Ultra VIP feature) live next to the database
// file, so on Railway they sit on the same persistent volume and survive
// redeploys instead of disappearing with the container.
...
// Safe incremental migration: add columns if this DB pre-dates them.
// Kept separate from state_json (unlike everything else) because videos are
// large — bundling them into the normal state blob would bloat every
// routine autosave and slow the whole app down.
function ensureColumn(table, column, def) {
  try { db.exec(`ALTER TABLE ${table} ADD COLUMN ${column} ${def}`); } catch (e) { /* already exists — fine */ }
}
ensureColumn('users', 'custom_bg_data', 'TEXT');
ensureColumn('users', 'custom_bg_type', 'TEXT');
...
app.post('/api/change-username', auth, (req, res) => {
  ...
  if (lower !== row.username_lower) {
    const exists = db.prepare('SELECT id FROM users WHERE username_lower = ?').get(lower);
    if (exists) return res.status(409).json({ error: 'That username is already taken.' });
  }
  db.prepare('UPDATE users SET username = ?, username_lower = ? WHERE id = ?').run(uname, lower, row.id);
  const updatedRow = { ...row, username: uname, username_lower: lower };
  const token = signToken(updatedRow);
  res.json({ ok: true, username: uname, token });
});

// Social login isn't wired up yet — these accounts have to actually be
// registered with Google/Facebook/X first (each needs its own developer
// app + OAuth client credentials from that platform). This endpoint just
// lets a player remove a placeholder link; the "Link" button itself tells
// them it's not configured yet until that setup is done.
app.post('/api/unlink-account', auth, (req, res) => {
  const { provider } = req.body || {};
  const row = db.prepare('SELECT * FROM users WHERE id = ?').get(req.user.uid);
  if (!row) return res.status(404).json({ error: 'Account not found.' });
  const state = JSON.parse(row.state_json);
  state.linkedAccounts = state.linkedAccounts || {};
  delete state.linkedAccounts[provider];
  db.prepare('UPDATE users SET state_json = ? WHERE id = ?').run(JSON.stringify(state), row.id);
  res.json({ ok: true });
});
...
// ---------------------------------------------------------------------------
// Custom background uploads (Ultra VIP feature — image or short video)
// ---------------------------------------------------------------------------
// Accepts a base64 data URL in JSON. Needs a larger body limit than the rest
// of the app, so it's declared with its own express.json() just for this route.
app.post('/api/upload-background', auth, express.json({ limit: '20mb' }), (req, res) => {
  const row = db.prepare('SELECT * FROM users WHERE id = ?').get(req.user.uid);
  if (!row) return res.status(404).json({ error: 'Account not found.' });
  const state = JSON.parse(row.state_json);
  if (!state.ultraVipUnlocked) return res.status(403).json({ error: 'Custom backgrounds require the Ultra VIP unlock first.' });

  const { dataUrl } = req.body || {};
  const match = /^data:(image|video)\/([a-zA-Z0-9.+-]+);base64,(.+)$/.exec(dataUrl || '');
  if (!match) return res.status(400).json({ error: 'Please upload a valid image or video file.' });
  const [, kind, subtype, base64] = match;
  const ext = subtype.split('+')[0].replace(/[^a-z0-9]/gi, '').slice(0, 8) || (kind === 'video' ? 'mp4' : 'jpg');
  const buffer = Buffer.from(base64, 'base64');
  const maxBytes = 15 * 1024 * 1024; // 15MB — generous for a short clip/photo without bloating the disk
  if (buffer.length > maxBytes) return res.status(400).json({ error: 'File is too large — please keep it under 15MB.' });

  // remove any previous upload for this user first (any extension)
  try {
    fs.readdirSync(UPLOAD_DIR).filter(f => f.startsWith(`bg_${row.id}_`)).forEach(f => fs.unlinkSync(path.join(UPLOAD_DIR, f)));
  } catch (e) {}

  const filename = `bg_${row.id}_${Date.now()}.${ext}`;
  fs.writeFileSync(path.join(UPLOAD_DIR, filename), buffer);

  state.customBackgroundUrl = `/uploads/${filename}`;
  state.customBackgroundType = kind; // 'image' | 'video'
  db.prepare('UPDATE users SET state_json = ?, updated_at = ? WHERE id = ?').run(JSON.stringify(state), Date.now(), row.id);
  res.json({ ok: true, url: state.customBackgroundUrl, type: kind });
});

app.post('/api/remove-background', auth, (req, res) => {
  const row = db.prepare('SELECT * FROM users WHERE id = ?').get(req.user.uid);
  if (!row) return res.status(404).json({ error: 'Account not found.' });
  const state = JSON.parse(row.state_json);
  try {
  ...
  delete state.customBackgroundUrl;
  delete state.customBackgroundType;
  db.prepare('UPDATE users SET state_json = ?, updated_at = ? WHERE id = ?').run(JSON.stringify(state), Date.now(), row.id);
  res.json({ ok: true });
});
...
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
// Custom backgrounds (Ultra VIP only) — stored separately from state_json
// ---------------------------------------------------------------------------

// Ultra VIP status lives inside state.vip.ultra (checked from the saved
// state, same as regular VIP), so no extra DB column needed for that part.
app.post('/api/upload-background', auth, express.json({ limit: '10mb' }), (req, res) => {
  const row = db.prepare('SELECT * FROM users WHERE id = ?').get(req.user.uid);
  if (!row) return res.status(404).json({ error: 'Account not found.' });
  const state = JSON.parse(row.state_json);
  if (!state.vip || !state.vip.ultra) return res.status(403).json({ error: 'Ultra VIP membership is required to upload a custom background.' });

  const { dataUrl, type } = req.body || {};
  if (!dataUrl || typeof dataUrl !== 'string') return res.status(400).json({ error: 'No file received.' });
  if (type !== 'image' && type !== 'video') return res.status(400).json({ error: 'Only image or video files are supported.' });
  // Rough size guard — base64 is ~33% larger than the original file, so this
  // caps the real file around ~7MB. Keeps the database and page loads sane.
  if (dataUrl.length > 9.5 * 1024 * 1024) return res.status(413).json({ error: 'File is too large. Please use something under 7MB.' });

  db.prepare('UPDATE users SET custom_bg_data = ?, custom_bg_type = ? WHERE id = ?').run(dataUrl, type, row.id);
  res.json({ ok: true });
});

app.get('/api/my-background', auth, (req, res) => {
  const row = db.prepare('SELECT custom_bg_data, custom_bg_type FROM users WHERE id = ?').get(req.user.uid);
  if (!row || !row.custom_bg_data) return res.json({ data: null, type: null });
  res.json({ data: row.custom_bg_data, type: row.custom_bg_type });
});

app.post('/api/remove-background', auth, (req, res) => {
  db.prepare('UPDATE users SET custom_bg_data = NULL, custom_bg_type = NULL WHERE id = ?').run(req.user.uid);
  res.json({ ok: true });
});
...
// Full read-only inspector for a single account: stats, inventory, equipped
// items, VIP status, and match history. Never includes the password hash.
app.get('/api/admin/user/:username', auth, requireAdmin, (req, res) => {
  const row = db.prepare('SELECT * FROM users WHERE username_lower = ?').get((req.params.username || '').toLowerCase());
  if (!row) return res.status(404).json({ error: 'User not found.' });
  const state = JSON.parse(row.state_json);
  res.json({
    ...
    level: state.level, xp: state.xp, coins: state.coins, gems: state.gems,
    stats: state.stats, vip: state.vip, ultraVipUnlocked: !!state.ultraVipUnlocked,
    equipped: state.equipped, inventory: state.inventory,
    achievements: state.achievements, campaign: state.campaign,
    matchHistory: (state.matchHistory || []).slice(-15).reverse()
  });
});
